...
**注意**：再创建一个新的线程时，需要判断该节点是否需要信息：
1. 若不需要，设置 'data_in_slice : [0:0]' 来使得线程不输入任何信息
2. 若需要，需要判断才哪一条线程获取信息，需要信息的切片是多少？
...
4. `llm-first` 中空的 `task_prompt` = 一个空节点，只会继承该线程输入的信息
5. `data_in_*` 仅在创建**新**线程时有效
